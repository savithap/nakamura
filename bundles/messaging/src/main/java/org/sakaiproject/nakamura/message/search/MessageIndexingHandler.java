    ...
    if (!StringUtils.isBlank(path)) {
      try {
        ...
        if (content != null) {
          ...
          if (quickIndex) {
            for (Entry<String, String> prop : QUICK_INDEX.entrySet()) {
              Object value = content.getProperty(prop.getKey());
              if (value != null) {
                doc.addField(prop.getValue(), value);
              }
            }
